...
function suggestion(name, profilePic, key) {
  database.ref(`friendship/${USER_ID}`).orderByChild('friendId').equalTo(key)
    .once('value', (snapshot) => {
      if (!snapshot.exists()) {
        if (key !== USER_ID) {
  ...
  database.ref(`friendship/${USER_ID}`).orderByChild('friendId').equalTo(key)
    .once('value', (snapshot) => {
      if (snapshot.exists()) {
        ...
        $(`button[data-friend-id=${key}]`).click(() => {
    ...
    .then(function(snapshot) {
      ...
      snapshot.forEach(function(childSnapshot) {
        ...
        if (email === temp[friendKey].email) {
          notFound = false;
          database.ref(`friendship/${USER_ID}`).orderByChild('friendId').equalTo(friendKey)
            .once('value', snapshot => {
              if (!snapshot.exists()) {
                $('#search').append(`
                            <li id="${friendKey}search">
                                <div class="container border border-light bg-light">
                                    <div class="row align-items-center justify-content-around">
                                        <div class="col-2 p-2 ml-n4">
                                            <figure class="background--gray rounded-circle profile-picture">
                                                <img class="w-100 rounded-circle margin-0 profile-picture" src="${temp[friendKey].imgURL}" alt="">
                                            </figure>
                                        </div>
                                        <div class="col-6 ml-n4">
                            ...
                            `)
                $(`button[data-friend-id=${friendKey}]`).click(() => {
                  database.ref(`friendship/${USER_ID}`).push({
                    friendId: friendKey
                  })
